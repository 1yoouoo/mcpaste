...
begin
    if exists (
        ...
        where revision_kind = 'attachment_bundle'
    ) then
        raise exception 'cannot rollback migration 000005 while attachment_bundle revisions exist';
    end if;
end;
$$;
...
alter table paste_revisions add constraint paste_revisions_body_fields check (
    ...
    or (revision_kind = 'image_bundle' and text_key_id is null and text_nonce is null and text_ciphertext is null)
);
