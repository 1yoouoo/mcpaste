...
The installer downloads [MCPaste v0.2.4](https://github.com/1yoouoo/mcpaste/releases/tag/v0.2.4) from GitHub Releases, verifies its SHA-256 checksum, installs it at `/Applications/MCPaste.app`, and links its embedded helper at `/usr/local/bin/mcpaste`. It prefers the notarized `MCPaste-final.zip` and falls back to the ad-hoc `MCPaste-adhoc.zip` when Apple signing credentials are unavailable.
...
For a manual install, download the macOS ZIP from the [v0.2.4 release](https://github.com/1yoouoo/mcpaste/releases/tag/v0.2.4), unzip `MCPaste.app` into `/Applications`, and open it. The one-line installer is recommended because it also installs the embedded MCP helper and verifies the release checksum.

Open MCPaste on each Mac. Restart the desired MCP client after registration so it loads the new MCP entry.

MCPaste appears as a regular app in `/Applications`, Launchpad, and the Dock.
