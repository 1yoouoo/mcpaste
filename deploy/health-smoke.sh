...
check_endpoint() {
	curl --fail --silent --max-time 10 "$endpoint/livez" >/dev/null 2>&1 || return 1
	curl --fail --silent --max-time 10 "$endpoint/readyz" >/dev/null 2>&1 || return 1
	if [[ -n "${MCPASTE_SMOKE_TOKEN:-}" ]]; then
		curl --fail --silent --max-time 10 --request POST \
			...
			"$endpoint/v1/mcp" >/dev/null 2>&1 || return 1
	fi
	return 0
}
